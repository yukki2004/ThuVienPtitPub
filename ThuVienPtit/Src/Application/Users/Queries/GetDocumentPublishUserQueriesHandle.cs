...
{
    public class GetDocumentPublishUserQueriesHandle : IRequestHandler<GetDocumentPublishUserQueries, GetDocumentUser>
    {
        private readonly IUserRespository _userRespository;
        private readonly string baseUrl;
        public GetDocumentPublishUserQueriesHandle(IUserRespository userRespository, IConfiguration configuration)
        {
            _userRespository = userRespository;
            baseUrl = configuration["FileStorage:BaseUrl"] ?? "";
        }
        public async Task<GetDocumentUser> Handle(GetDocumentPublishUserQueries request, CancellationToken cancellationToken)
        {
            var documents = await _userRespository.GetUserInfoByIdPublishAsync(request.user_id, request.pageNumber, cancellationToken);
