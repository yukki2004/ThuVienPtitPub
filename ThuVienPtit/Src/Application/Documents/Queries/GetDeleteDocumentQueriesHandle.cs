...
{
    public class GetDeleteDocumentQueriesHandle : IRequestHandler<GetDeleteDocumentQueries, GetDocumentRespone>
    {
        private readonly Interface.IDocumentRespository documentRespository;
        private readonly string basUrl;
        private readonly ICacheService cacheService;
        private const string VersionKey = "DeleteDoc:version";
        public GetDeleteDocumentQueriesHandle(Interface.IDocumentRespository documentRespository, IConfiguration configuration, ICacheService cacheService)
        {
            this.documentRespository = documentRespository;
            basUrl = configuration["FileStorage:BaseUrl"] ?? "";
            this.cacheService = cacheService;
        }
        public async Task<GetDocumentRespone> Handle(GetDeleteDocumentQueries request, CancellationToken cancellationToken)
        {
            var version = await cacheService.GetVersionAsync(VersionKey) ?? 1;
            var cacheKey = $"DeleteDoc:{version}:page:{request.pageNumber}";
            ...
            var document = await documentRespository.GetDeleteDocumentAdminAsync(request.pageNumber);
